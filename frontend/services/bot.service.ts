...
export const watsonxService = {
  ...
  getInsightDetails: async (
    insightId: string,
  ): Promise<WatsonXInsight & { detailed_analysis: any; recommended_actions: string[] }> => {
    const response = await botApi.get(`/admin/analytics/watsonx/insights/${insightId}`)
    return response.data
  },

  // Analyze current data with WatsonX
  analyzeCurrentData: async (request: WatsonXAnalysisRequest): Promise<any> => {
    const response = await botApi.post("/admin/analytics/watsonx/analyze", request)
    return response.data
  },
}
